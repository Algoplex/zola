...
export function SidebarProjectItem({ project }: SidebarProjectItemProps) {
  ...
  const containerClassName = useMemo(
    () =>
      cn(
        "hover:bg-sidebar-accent/80 hover:text-sidebar-foreground group/project relative w-full rounded-md transition-colors",
    ...
    >
      {isEditing ? (
        <div className="bg-sidebar-accent flex items-center rounded-md py-1 pr-1 pl-2">
          <FolderIcon
            size={20}
            className="text-sidebar-foreground mr-2 flex-shrink-0"
          />
          <input
            ref={inputRef}
            value={editName}
        ...
        <>
          ...
          >
            <div
              className="text-sidebar-foreground/90 relative line-clamp-1 flex w-full items-center gap-2 mask-r-from-80% mask-r-to-85% px-2 py-2 text-sm text-ellipsis whitespace-nowrap"
              title={displayName}
            >
              <FolderIcon size={20} />
              {displayName}
